...
function createBasicTemplateStructure(templatesDir, templateName, structure) {
...
function createStructureRecursive(baseDir, structure) {
    for (const [name, content] of Object.entries(structure)) {
        ...
        // Otherwise it's a directory with more structure
            ...
            createStructureRecursive(itemPath, content);
        }
    }
}
...
function activate(context) {
    ...
    const copyTemplateItemCommand = vscode.commands.registerCommand('project-template-manager.copyTemplateItem', async (node) => {
        ...
        try {
            ...
            if (fs.existsSync(targetPath)) {
                const overwrite = await vscode.window.showWarningMessage(`"${itemName}" already exists in your workspace. Overwrite?`, { modal: true }, 'Overwrite');
                if (overwrite !== 'Overwrite') {
